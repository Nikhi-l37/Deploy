...
import { 
  Activity, Zap, Globe, ArrowRight, Layers, Lock, ChevronDown, RefreshCw
} from 'lucide-react';
...
const PIPELINE_LINES = [
  ...
  { ts: '00:01', prefix: '✓', action: 'GitHub Webhook received and verified', target: 'HMAC-SHA256', delay: 300, badge: 'verified', badgeColor: 'green' },
  { ts: '00:02', prefix: '✓', action: 'Build job dispatched to background worker', target: 'Redis Stream', delay: 250, badge: 'queued', badgeColor: 'amber' },
  { ts: '00:03', prefix: '✓', action: 'Cloning repository into build workspace', target: 'depth=1', delay: 350, badge: 'git clone', badgeColor: 'gray' },
  { ts: '00:04', prefix: '✓', action: 'Runtime auto-detected', target: 'Node.js 20 LTS (Fastify + React SSR)', delay: 300, badge: 'Node.js 20', badgeColor: 'blue' },
  { ts: '00:05', prefix: '✓', action: 'Multi-stage Dockerfile generated', target: 'Alpine 3.19 base', delay: 300, badge: 'multi-stage', badgeColor: 'purple' },
  { ts: '00:07', prefix: '✓', action: 'Compiling container image & caching build layers', target: 'docker build', delay: 600, badge: 'cached', badgeColor: 'cyan' },
  { ts: '00:10', prefix: '✓', action: 'Allocating sandbox', target: '512MB RAM cap & dedicated CPU core', delay: 300, badge: 'cgroups v2', badgeColor: 'gray' },
  { ts: '00:12', prefix: '✓', action: 'Decrypting .env secrets into container environment', target: 'Fernet AES-128 GCM', delay: 300, badge: 'AES-128', badgeColor: 'purple' },
  { ts: '00:13', prefix: '✓', action: 'Container running in sandbox and bound to port', target: ':8008', delay: 300, badge: 'healthy', badgeColor: 'green' },
  { ts: '00:14', prefix: '✓', action: 'Nginx edge reverse proxy reloaded & SSL certificate provisioned', target: 'HTTP/2 SSL', delay: 250, badge: 'active', badgeColor: 'green' },
...
const getBadgeStyle = (color) => {
  switch (color) {
    ...
    case 'cyan':
      return 'bg-[#39c5cf]/15 text-[#56d4dd] border-[#39c5cf]/40';
    default:
...
export default function Login() {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-[#010409] text-[#c9d1d9] antialiased">
      ...
      <main className="flex-1 flex flex-col items-center justify-start px-4 pt-2 sm:pt-4 pb-8 max-w-6xl mx-auto w-full">
        <div className="w-full space-y-6 sm:space-y-8 animate-fade-in text-center">
            
            {/* Hero Section */}
            <section className="relative flex flex-col items-center justify-start text-center min-h-[calc(100vh-70px)] pt-6 sm:pt-10 pb-16 space-y-6 sm:space-y-7 max-w-4xl mx-auto">
              {/* Subtle ambient radial glow behind hero */}
              <div 
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[550px] sm:w-[700px] h-[300px] sm:h-[380px] bg-gradient-to-tr from-[#238636]/15 via-[#2ea44f]/10 to-transparent blur-[90px] pointer-events-none -z-10 rounded-full" 
                aria-hidden="true"
              />

              {/* Badge */}
              <div className="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-[#161b22]/90 backdrop-blur-sm border border-[#30363d] text-xs sm:text-sm text-[#8b949e] font-mono shadow-sm hover:border-[#8b949e]/50 transition-colors">
              ...
              <h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold tracking-[-0.035em] text-[#ffffff] leading-[1.15] sm:leading-[1.12]">
                ...
                <span className="relative inline-block text-transparent bg-clip-text bg-gradient-to-r from-[#3fb950] via-[#56d364] to-[#3fb950] drop-shadow-[0_0_24px_rgba(63,185,80,0.35)]">
                  code
                </span>{' '}
                to production.
              </h1>
              ...
              <p className="text-base sm:text-xl lg:text-2xl text-[#8b949e] max-w-3xl mx-auto leading-relaxed font-normal">
                Connect your repository, configure environment secrets, and Deployat automatically builds{' '}
                <span className="text-[#c9d1d9] font-medium">Docker containers</span> and generates{' '}
                <span className="text-[#c9d1d9] font-medium">Nginx reverse proxy</span> routes in seconds.
              </p>
              ...
              <div className="pt-2 flex flex-wrap items-center justify-center gap-4">
                <button
                  onClick={handleGithubLogin}
                  disabled={isLoading}
                  className="flex items-center gap-3 bg-[#238636] hover:bg-[#2ea043] active:bg-[#29903b] text-[#ffffff] font-semibold py-3.5 px-8 rounded-[8px] border border-[rgba(240,246,252,0.1)] shadow-[0_2px_12px_rgba(35,134,54,0.35)] transition-all transform hover:-translate-y-0.5 text-base cursor-pointer disabled:opacity-50"
                >
                ...
                <button 
                  onClick={() => {
                    const el = document.getElementById('architecture-section') || document.getElementById('live-preview-section');
            ...
            {/* ========================================================== */}
            <div id="live-preview-section" className="max-w-5xl mx-auto w-full pt-6 sm:pt-8 scroll-mt-20">
                ...
                <div className="bg-[#161b22]/95 backdrop-blur border-b border-[#30363d] px-4 py-3 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-[11px] font-mono text-[#8b949e]">
                    {terminalDone && (
                      <button 
                        onClick={runTerminal}
                        className="flex items-center gap-1.5 text-[#8b949e] hover:text-[#f0f6fc] text-[11px] px-2.5 py-1 rounded bg-[#21262d] border border-[#30363d] hover:border-[#8b949e] cursor-pointer transition-colors"
                      >
                    ...
                    >
                      {line.type === 'command' ? (
                        ...
                        <div className="flex items-center justify-between gap-3 py-0.5 hover:bg-[#161b22]/30 px-1 rounded transition-colors">
                          ...
                          {line.badge && (
                            <span className={`px-1.5 py-0.5 rounded text-[10.5px] font-mono border font-normal shrink-0 ${getBadgeStyle(line.badgeColor)}`}>
                              {line.badge}
                            </span>
                          )}
                        </div>
            ...
            <div id="architecture-section" className="w-full">
              <ArchitectureCanvas />
            </div>
            ...
            <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-4 text-left">
              ...
              <div className="bg-[#161b22] border border-[#30363d] hover:border-[#444c56] rounded-[6px] p-4 space-y-1.5 transition-colors duration-150 cursor-default shadow-sm">
                ...
                <p className="text-[11px] text-[#8b949e] leading-relaxed">
                  Protected with 512MB RAM caps and dedicated CPU cores to prevent runaway processes.
                </p>
              </div>
            </div>
          </div>
